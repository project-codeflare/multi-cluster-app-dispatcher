...
do
    next_num=$((num + 1))
    kubectl apply -f "$output_dir/stress-mcadkwok-cpu-j-s-$next_num.yaml"
done


# Check for all jobs to report complete
jobstatus=`kubectl get jobs -n default --no-headers --field-selector status.successful=$awjobs |grep mcadkwok | wc -l`
...
echo "Total amount of time for $jobs fake jobs with MCAD is: $SECONDS seconds" 
