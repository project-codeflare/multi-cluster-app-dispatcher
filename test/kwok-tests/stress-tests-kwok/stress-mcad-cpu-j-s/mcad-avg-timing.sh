...
function help() {
    ...
    echo
    echo "Description: Runs performance test script(s) with MCAD in subdirectories under $SCRIPT_DIR."
...
extract_total_time() {
    awk '/fake jobs with MCAD is: .* seconds/ {print $(NF-1)}' "$1"
}
...
for num in $(eval echo "{0.."$realjobs"}"); do
  next_num=$((num + 1))
  cp stress-mcadkwok-cpu-j-s.yaml "$output_dir/stress-mcadkwok-cpu-j-s-$next_num.yaml"

  # Had to do this OSTYPE because sed acts differently on Linux versus Mac
    case "$OSTYPE" in
      linux-gnu*)
        sed -i "s/mcadkwok-cpu-job-short-0/mcadkwok-cpu-job-short-$next_num/g" "$output_dir/stress-mcadkwok-cpu-j-s-$next_num.yaml"
        sed -i "s/nvidia.com\/gpu: 0/nvidia.com\/gpu: $gpus/g" "$output_dir/stress-mcadkwok-cpu-j-s-$next_num.yaml"
        sed -i "s/parallelism: 1/parallelism: $awjobs/g" "$output_dir/stress-mcadkwok-cpu-j-s-$next_num.yaml"
        sed -i "s/completions: 1/completions: $awjobs/g" "$output_dir/stress-mcadkwok-cpu-j-s-$next_num.yaml" ;;
      darwin*) 
        sed -i '' "s/mcadkwok-cpu-job-short-0/mcadkwok-cpu-job-short-$next_num/g" "$output_dir/stress-mcadkwok-cpu-j-s-$next_num.yaml"
        sed -i '' "s/nvidia.com\/gpu: 0/nvidia.com\/gpu: $gpus/g" "$output_dir/stress-mcadkwok-cpu-j-s-$next_num.yaml"
        sed -i '' "s/parallelism: 1/parallelism: $awjobs/g" "$output_dir/stress-mcadkwok-cpu-j-s-$next_num.yaml"
...
for ((i=1; i<=iterations; i++)); do
    ...
    output_file=$(mktemp)
    echo "Now calling mcadkwok-stress-test.sh"
    echo " "
    ./mcadkwok-stress-test.sh -j "$jobs" -g "$gpus" -a "$awjobs" > "$output_file"
    ...
    sleep 10
    kubectl scale deployment mcad-controller --replicas=1 -n kube-system
    sleep 5
done
