...
function help() {
    echo "usage: baseline-avg-timing.sh [-h] [-j <jobs>] [-g <gpus>] [-a <awjobs>]"
    echo
    echo "Description: Runs performance test script(s) without MCAD in subdirectories under $SCRIPT_DIR."
    echo "NOTE: This runs on KIND nodes only."
    echo
    echo "Preconditions: "
    ...
    echo "  -j <jobs>       Number of jobs to run (default: 1)"
...
extract_total_time() {
    awk '/real jobs without MCAD is: .* seconds/ {print $(NF-1)}' "$1"
}
...
for num in $(eval echo "{0.."$realjobs"}"); do
  next_num=$((num + 1))
  cp stress-baseline-cpu-j-s.yaml "$output_dir/stress-baseline-cpu-j-s-$next_num.yaml"

  # Had to do this OSTYPE because sed acts differently on Linux versus Mac
    case "$OSTYPE" in
      linux-gnu*)
        sed -i "s/baseline-cpu-job-short-0/baseline-cpu-job-short-$next_num/g" "$output_dir/stress-baseline-cpu-j-s-$next_num.yaml"
        sed -i "s/nvidia.com\/gpu: 0/nvidia.com\/gpu: $gpus/g" "$output_dir/stress-baseline-cpu-j-s-$next_num.yaml"
        sed -i "s/parallelism: 1/parallelism: $awjobs/g" "$output_dir/stress-baseline-cpu-j-s-$next_num.yaml"
        sed -i "s/completions: 1/completions: $awjobs/g" "$output_dir/stress-baseline-cpu-j-s-$next_num.yaml" ;;
      darwin*) 
        sed -i '' "s/baseline-cpu-job-short-0/baseline-cpu-job-short-$next_num/g" "$output_dir/stress-baseline-cpu-j-s-$next_num.yaml"
        sed -i '' "s/nvidia.com\/gpu: 0/nvidia.com\/gpu: $gpus/g" "$output_dir/stress-baseline-cpu-j-s-$next_num.yaml"
        sed -i '' "s/parallelism: 1/parallelism: $awjobs/g" "$output_dir/stress-baseline-cpu-j-s-$next_num.yaml"
...
for ((i=1; i<=iterations; i++)); do
    ...
    output_file=$(mktemp)
    echo "Now calling baseline-stress-test.sh"
    echo " "
    ./baseline-stress-test.sh -j "$jobs" -g "$gpus" -a "$awjobs" > "$output_file"
    ...
    ./cleanup-baseline-cpu-j-s.sh > "$output_file"
    rm "$output_file"
    echo " "
    sleep 10
done
