...
function help() {
    echo "usage: stress-test [-h] [-j <jobs>] [-g <gpus>]"
    echo
...
for num in $(eval echo "{0.."$realjobs"}"); do
  ...
  # Generate a random number for awjobs using Gaussian distribution
  mean=10
  std_dev=5
  awjobs=$(python3 -c "import random; print(round(abs(random.gauss($mean, $std_dev))))")
