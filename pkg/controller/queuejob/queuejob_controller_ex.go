...
// To get the aggregate resources of all pods
...
// To assign an agent (i.e. cluster) that fits the resource criteria as specified in AppWrapper
// If multiple agents exist, the selection is done randomly
func (qjm *XController) chooseAgent(qj *arbv1.AppWrapper) string {

	// Set aggregate resources of the queued job as specified in AppWrapper
	...
	// Set the unallocated resource of the randomly selected agent
	unallocatedResources := qjm.agentMap[agentId].AggrResources

	// Set the priority of the AppWrapper with respect to its status (Pending/Running/Failed, etc.)
