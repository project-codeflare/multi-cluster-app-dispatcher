...
func (qjm *XController) chooseAgent(qj *arbv1.AppWrapper) string {
	...
	klog.V(2).Infof("[Controller: Dispatcher Mode] Aggr Resources of XQJ %s: %v\n", qj.Name, qjAggrResources)

	agentId := qjm.agentList[rand.Int()%len(qjm.agentList)]
	klog.V(2).Infof("[Controller: Dispatcher Mode] Agent %s is chosen randomly\n", agentId)
	resources := qjm.agentMap[agentId].AggrResources
	klog.V(2).Infof("[Controller: Dispatcher Mode] Aggr Resources of Agent %s: %v\n", agentId, resources)
	if qjAggrResources.LessEqual(resources) {
		klog.V(2).Infof("[Controller: Dispatcher Mode] Agent %s has enough resources\n", agentId)
		return agentId
	}
	klog.V(2).Infof("[Controller: Dispatcher Mode] Agent %s does not have enough resources\n", agentId)
...
func (qjm *XController) ScheduleNext() {
	...
	if qjm.serverOption.DynamicPriority {
		...
		qj.Status.SystemPriority = qj.Spec.PrioritySlope + qj.Spec.PrioritySlope*(time.Now().Sub(qj.Status.ControllerFirstTimestamp.Time)).Seconds()
		tempQ.Add(qj)
		for qjm.qjqueue.Length() > 0 {
			qjtemp, _ := qjm.qjqueue.Pop()
			qjtemp.Status.SystemPriority = qjtemp.Spec.PrioritySlope + qjtemp.Spec.PrioritySlope*(time.Now().Sub(qjtemp.Status.ControllerFirstTimestamp.Time)).Seconds()
			tempQ.Add(qjtemp)
		}
			...
			if aggqj.LessEqual(resources) {
				...
				if err := qjm.updateEtcd(qj, "ScheduleNext - setCanRun"); err == nil {
					...
					if err = qjm.enqueue(qj); err != nil { // unsuccessful add to eventQueue, add back to activeQ
						...
						forwarded = true
						klog.V(3).Infof("[ScheduleNext] %s Delay=%.6f seconds eventQueue.Add_afterHeadOfLine activeQ=%t, Unsched=%t &qj=%p Version=%s Status=%+v", qj.Name, time.Now().Sub(qj.Status.ControllerFirstTimestamp.Time).Seconds(), qjm.qjqueue.IfExistActiveQ(qj), qjm.qjqueue.IfExistUnschedulableQ(qj), qj, qj.ResourceVersion, qj.Status)
					}
				}
...
func (cc *XController) updateEtcd(qj *arbv1.AppWrapper, at string) error {
	...
	if qjj, err := cc.arbclients.ArbV1().AppWrappers(qj.Namespace).Update(qj); err != nil {
		klog.Errorf("[updateEtcd] Failed to update status of AppWrapper %s, namespace: %s at %s err=%v", qj.Name, qj.Namespace, at, err)
		return err
	...
	klog.V(10).Infof("[updateEtcd] AppWrapperUpdate success %s at %s &qj=%p qj=%+v", qj.Name, at, qj, qj)
...
func (qjm *XController) backoff(q *arbv1.AppWrapper, reason string, message string) {
	...
	klog.V(3).Infof("[backoff] %s move to unschedulableQ before sleep for %d seconds. activeQ=%t Unsched=%t &qj=%p Version=%s Status=%+v", q.Name, qjm.serverOption.BackoffTime, qjm.qjqueue.IfExistActiveQ((q)), qjm.qjqueue.IfExistUnschedulableQ((q)), q, q.ResourceVersion, q.Status)
	...
	klog.V(3).Infof("[backoff] %s activeQ.Add after sleep for %d seconds. activeQ=%t Unsched=%t &qj=%p Version=%s Status=%+v", q.Name, qjm.serverOption.BackoffTime, qjm.qjqueue.IfExistActiveQ((q)), qjm.qjqueue.IfExistUnschedulableQ((q)), q, q.ResourceVersion, q.Status)
}
...
func (qjm *XController) UpdateQueueJobs() {
	...
	for _, newjob := range queueJobs {
		...
		if newjob.Status.QueueJobState == "" {
			...
			newjob.Status.SystemPriority = newjob.Spec.PrioritySlope
...
func (cc *XController) addQueueJob(obj interface{}) {
	...
	if qj.Status.QueueJobState == "" {
		...
		qj.Status.SystemPriority = qj.Spec.PrioritySlope
...
func (cc *XController) deleteQueueJob(obj interface{}) {
	...
	klog.V(3).Infof("[Informer-deleteQJ] %s *Delay=%.6f seconds before enqueue &qj=%p Version=%s Status=%+v", qj.Name, time.Now().Sub(qj.Status.ControllerFirstTimestamp.Time).Seconds(), qj, qj.ResourceVersion, qj.Status)
	cc.enqueue(qj)
}
...
func (cc *XController) syncQueueJob(qj *arbv1.AppWrapper) error {
	...
	if larger(queueJob.ResourceVersion, qj.ResourceVersion) {
		klog.V(10).Infof("[worker-syncQJ] %s found more recent copy from cache       &qj=%p       qj=%+v", qj.Name, qj, qj)
		klog.V(10).Infof("[worker-syncQJ] %s found more recent copy from cache &queueJob=%p queueJob=%+v", queueJob.Name, queueJob, queueJob)
...
func (cc *XController) manageQueueJob(qj *arbv1.AppWrapper) error {
	...
	if !cc.isDispatcher { // Agent Mode
		...
		if !qj.Status.CanRun && (qj.Status.State != arbv1.AppWrapperStateEnqueued && qj.Status.State != arbv1.AppWrapperStateDeleted) {
			...
			// pending state...
			klog.V(2).Infof("[Agent Mode] Deleting resources for XQJ %s because it will be preempted (newjob)\n", qj.Name)
			err = cc.Cleanup(qj)
			if err != nil {
				return err
			}
		...
		if !qj.Status.CanRun && (qj.Status.State != arbv1.AppWrapperStateEnqueued && qj.Status.State != arbv1.AppWrapperStateDeleted) {
			// if there are running resources for this job then delete them because the job was put in
			// pending state...
			klog.V(3).Infof("Deleting queuejob resources because it will be preempted! %s", qj.Name)
			err = cc.Cleanup(qj)
			if err != nil {
				return err
			}
		...
		if qj.Status.CanRun && !qj.Status.IsDispatched {
			...
			if klog.V(10) {
				...
				klog.V(10).Infof("[Dispatcher Controller] XQJ %s has Overhead Before Dispatching: %s", qj.Name, current_time.Sub(qj.CreationTimestamp.Time))
...
func (cc *XController) Cleanup(queuejob *arbv1.AppWrapper) error {
	...
	klog.V(3).Infof("[Cleanup] end   AppWrapper %s Version=%s Status=%+v\n", queuejob.Name, queuejob.ResourceVersion, queuejob.Status)
