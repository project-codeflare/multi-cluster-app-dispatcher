...
const AppWrapperCrdPath = "config/crd/bases/mcad.ibm.com_appwrappers.yaml"
...
func CreateAppWrapperKind(clientset apiextensionsclient.Interface) (*apiextensionsv1.CustomResourceDefinition, error) {
	f, err := os.ReadFile(AppWrapperCrdPath)
	if err != nil {
		return nil, fmt.Errorf("cannot load the CRD from file %s: %v", AppWrapperCrdPath, err)
	}
